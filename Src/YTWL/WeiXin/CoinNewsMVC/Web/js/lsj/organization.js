...
//请求地址(列表)
var RequestListUrl = "/Organization/GetOrganizationList";
//请求参数(关联上级标识)
var RequestTopID = "id";
//请求地址(下级列表)
var RequestSubListUrl = "/Organization/GetOrganizationSubList"
//请求地址(单条记录)
var RequestUrl = "/Organization/GetOrganization";
//请求地址(删除记录)
var RequestDelUrl = "/Organization/DelOrganization";
//请求控制器(修改)
var RequestEditUrl = "/Organization/EditOrganization";
//请求控制器(添加)
var RequestAddUrl = "/Organization/AddOrganization"
...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        $("#btn_add").click(function () {
            ...
            $('#txt_parent_id').selectpicker('val', 0);
            $('#txt_orgtype_id').selectpicker('val', 1);
            ...
            if (arrselections.length == 1) {
                if (arrselections[0].levels > 1) {
                    ...
                    $('#txt_parent_id').selectpicker('val', arrselections[0].id);
                }
            }
        ...
        $("#btn_edit").click(function () {
            ...
            $.post(RequestUrl, { id: arrselections[0].id }, function (data) {
                ...
                $('#txt_parent_id').selectpicker('val', data.parent_id);
                $('#txt_orgtype_id').selectpicker('val', data.orgtype.id);
...
function openFile(t) {
    ...
    if ($(t).attr("class") == "fa fa-folder") {
        ...
        if (index == null) {
            loading.show();
            $.post("/Organization/GetOrganizationSubList", { parent_id: id }, function (data) {
                loading.close();
                eacheDate = data;
