...
namespace DAO.DAL
{
	...
	/// 接口层D_Manager
	/// </summary>
	public class D_Manager:IManager
	{
		...
		/// 获得前几行数据
		/// </summary>
		public virtual IList<Manager>  GetList(List<SearchTemplate> st, List<Order> order)
		{
			using (ISession session = sessionFactory.OpenSession())
            {
                ICriteria crit = session.CreateCriteria(typeof(Manager));
                IList<Manager> customers = ManagerPage.GetCrit<Manager>(st, order, crit);
                return customers;
            }
		}
