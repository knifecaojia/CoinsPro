...
namespace DAO.DAL {
	...
	public class D_Users:IUsers
	{
		...
		/// 获得前几行数据
		/// </summary>
        public virtual IList<Users> GetList(int pageIndex = 1, int pageSize = 10)
		{
			using (ISession session = sessionFactory.OpenSession())
            {
                ...
                IList<Users> customers = crit.List<Users>();
                return customers;
            }
		}
        ...
        /// <returns></returns>
        public virtual IList<Users> GetList(List<SearchTemplate> st, List<Order> order)
        {
            using (ISession session = sessionFactory.OpenSession())
            {
                ICriteria crit = session.CreateCriteria(typeof(Users));
