...
namespace DAO.Cache {
	/// <summary>
	/// 缓存层D_Organization_type
	/// </summary>
	public class C_Organization_type:D_Organization_type
	{
        //类名 + 方法名 + 参数
		...
		{
			string key = "Organization_type:" + id;
		...
		/// 得到一个对象实体
		/// </summary>
		public override Organization_type Get(object id)
		{
			string key = "Organization_type:" + id;
            if (rh.Exist(key) > 0) //存在
            {
                return rh.Get<Organization_type>(key);
            }
            else
            {
                var model = base.Get(id);
		...
		/// 获得数据列表
		/// </summary>
		public override IList<Organization_type> LoadAll()
		{
			var functionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
            var key = className + ":" + functionName;
            //添加关联
            rh.AddJoin("Organization_type", key);
            if (rh.Exist(key) > 0) //存在
            {
                return rh.GetList<Organization_type>(key) as IList<Organization_type>;
            }
            else 
            {
                IList<Organization_type> list = base.LoadAll();
                //写入缓存
                rh.AddList<Organization_type>(key, list);
                return list;
            }
		}

		public override IList<Organization_type>  GetList(List<SearchTemplate> st, List<Order> order)
		{
			    var functionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
	            ...
	            {
	                IList<Organization_type> list = base.GetList(st, order);
	                //写入缓存
	                rh.AddList<Organization_type>(key, list);
	                return list;
	            }
