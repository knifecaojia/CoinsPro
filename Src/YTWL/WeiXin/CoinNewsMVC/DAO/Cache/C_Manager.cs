...
namespace DAO.Cache {
	...
	/// 缓存层D_Manager
	/// </summary>
	public class C_Manager:D_Manager
	{
        //类名 + 方法名 + 参数
		...
		/// 增加一条数据
		/// </summary>
		public override object  Save(Manager model)
		{
			var res = base.Save(model);
            if ((int)res > 0) 
            {
                string key = "Manager:" + res;
		...
		/// 删除数据
		/// </summary>
		public override void Delete(Manager model)
		{
			 string key = "Manager:" + model.id;
            if (rh.Exist(key) > 0) //存在
            {
		...
		/// 得到一个对象实体
		/// </summary>
		public override Manager Get(object id)
		{
			string key = "Manager:" + id;
            if (rh.Exist(key) > 0) //存在
            {
                return rh.Get<Manager>(key);
            }
            else
            {
                var model = base.Get(id);
		...
		/// 获得数据列表
		/// </summary>
		public override IList<Manager> LoadAll()
		{
			var functionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
            var key = className + ":" + functionName;
            //添加关联
            rh.AddJoin("Manager", key);
            if (rh.Exist(key) > 0) //存在
            {
                return rh.GetList<Manager>(key) as IList<Manager>;
            }
            else 
            {
                IList<Manager> list = base.LoadAll();
                //写入缓存
                rh.AddList<Manager>(key, list);
                return list;
            }
		}

		public override IList<Manager>  GetList(List<SearchTemplate> st, List<Order> order)
		{
			    var functionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
	            ...
	            {
	                IList<Manager> list = base.GetList(st, order);
	                //写入缓存
	                rh.AddList<Manager>(key, list);
	                return list;
	            }
