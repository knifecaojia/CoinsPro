...
namespace DAO.Cache {
	...
	public class C_News:D_News
	{
		...
		/// 增加一条数据
		/// </summary>
		public override object  Save(News model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public override void Update(News model)
		{
			string key = "News:" + model.id;
		...
		/// 得到一个对象实体
		/// </summary>
		public override News Get(object id)
		{
			string key = "News:" + id;
            if (rh.Exist(key) > 0) //存在
            {
                return rh.Get<News>(key);
            }
            else
            {
                var model = base.Get(id);
		...
		/// 获得数据列表
		/// </summary>
		public override IList<News> LoadAll()
		{
            ...
            if (rh.Exist(key) > 0) //存在
            {
                return rh.GetList<News>(key) as IList<News>;
            }
            else 
            {
                IList<News> list = base.LoadAll();
                //写入缓存
                rh.AddList<News>(key, list);
                return list;
            }
		}

		public override IList<News>  GetList(List<SearchTemplate> st, List<Order> order)
		{
			    var functionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
	            ...
	            {
	                IList<News> list = base.GetList(st, order);
	                //写入缓存
	                rh.AddList<News>(key, list);
	                return list;
	            }
