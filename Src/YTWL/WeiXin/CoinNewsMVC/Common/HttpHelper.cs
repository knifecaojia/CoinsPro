namespace MyHttp
{
    ...
    public class HttpHelper
    {
        ...
        private void SetEncoding(HttpItem item, HttpResult result, byte[] ResponseByte)
        {
            ...
            if (encoding == null)
            {
                System.Text.RegularExpressions.Match meta = System.Text.RegularExpressions.Regex.Match(Encoding.Default.GetString(ResponseByte), "<meta[^<]*charset=([^<]*)[\"']", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        ...
        private byte[] GetByte()
        {
            ...
            using (MemoryStream _stream = new MemoryStream())
            {
                ...
                {
                    //开始读取流并设置编码方式
                    new System.IO.Compression.GZipStream(response.GetResponseStream(), System.IO.Compression.CompressionMode.Decompress).CopyTo(_stream, 10240);
                }
                else
                {
                    //开始读取流并设置编码方式
        ...
        private void SetCer(HttpItem item)
        {
            if (!string.IsNullOrWhiteSpace(item.CerPath))
            {
                ...
                SetCerList(item);
                //将证书添加到请求里
                request.ClientCertificates.Add(new System.Security.Cryptography.X509Certificates.X509Certificate(item.CerPath));
            }
            else
            {
                //初始化对像，并设置请求的URL地址
        ...
        private bool CheckValidationResult(object sender, X509Certificate certificate, X509Chain chain, System.Net.Security.SslPolicyErrors errors) { return true; }
    ...
    public class HttpExample
    {
        ...
        public string GetHtml(string url)
        {
            ...
            HttpResult result = Spider.GetHtml(item);
            string html = result.Html;
            return html;
        }
        #endregion

        #region GetWebImage() 获取网页中的图片
        public static void GetWebImage(string imgUrl, string savePath)
        {
            var imgBytes = GetWebImage(imgUrl);
        ...
        public static byte[] GetWebImage(string imgUrl)
        {
            HttpHelper Spider = new HttpHelper();
            HttpResult result = Spider.GetHtml(new HttpItem()
            {
                URL = imgUrl,//URL 
        ...
        #region GetCodeImage() 获取验证码图片
        /// <summary>
        /// 获取验证码
        /// </summary>
        /// <param name="codeUrl">验证码图片地址</param>
        /// <param name="SetCookie">验证码cookie</param>
        /// <returns></returns>
        public static Image GetCodeImage(string codeUrl, out string SetCookie)
        {
            SetCookie = null;
            HttpHelper Spider = new HttpHelper();
            HttpItem item = new HttpItem()
            ...
            //获取cookie,如果是验证码要顺便获取ASPNET_SessionId，登陆的时候会用到
            string sr = result.Cookie;
            byte[] Bytes = result.ResultByte;
            //将返回的二进制数组转换为流
