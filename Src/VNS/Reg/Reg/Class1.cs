...
namespace Reg
{
    ...
    /// 发布时间：217-12-11
    /// </summary>
    public class YiMaSmsService: SmsInterface
    {
        public static string API = "http://api.fxhyd.cn/UserInterface.aspx";//易码短信服务平台开放接口地址
        public static string Token = "";//通信令牌，可登录网站查看，也可通过调用登录接口获取
        public string ServiceName { get { return "易码"; } }
        public static string username = "knifeandcj";//帐号
        public static string password = "knifecaojia";//密码

        public int itemid = 13950;//项目编号，必须参数
        ...
        public YiMaSmsService(string pid)
        {
            Pid = pid;
        }
        /// <summary>
        ...
        public bool Login(out string msg)
        {
            ...
            try
            {
                var result = DoGet(string.Format("action=login&username={0}&password={1}", username, password));
        ...
        public bool Mobile_Get(out string msg)
        {
            ...
            string _url = string.Format("action=getmobile&itemid={0}", itemid);
            if (province > 0)
        ...
        {
            msg = string.Empty;
            var result = DoGet(string.Format("action=addignore&mobile={0}&itemid={1}", mobileno, itemid));
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
        ...
        {
            msg = string.Empty;
            var result = DoGet(string.Format("action=release&mobile={0}&itemid={1}", mobileno, itemid));
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                if (result.Html.Trim().ToLower().Equals("success"))
                {
        ...
        {
            msg = string.Empty;
            var result = DoGet(string.Format("action=getsms&itemid={0}&mobile={1}", itemid, mobileno));
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                if (result.Html.Trim().ToLower().IndexOf("success|") == 0)
                {
