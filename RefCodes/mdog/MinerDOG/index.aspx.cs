...
namespace MinerDOG
{
    public partial class index : System.Web.UI.Page
    {
        ...
        private string ConvertHashrate(double d,double price,bool money=false,bool usd=false)
        {
            double k = 1000;
            double m = k * 1000;
            double g = m * 1000;
                        ...
                        if (d > t)//t
                        {
                            ...
                            else
                            {
                                if (money)
                                {
                                    ...
                                    try
                                    {
                                        if (usd)
                                        {
                                            try
                                            {
                                                ...
                                                string rawdata = "";
                                                rawdata = GetWebRequest(url);
                                                double usdprice = Convert.ToDouble(JObject.Parse(rawdata)["bid"]);

                                                url = "http://api.fixer.io/latest";
                                                rawdata = GetWebRequest(url);
                                                double eur2usd = Convert.ToDouble(JObject.Parse(rawdata)["rates"]["USD"].ToString()); ;
                                                double eur2cny = Convert.ToDouble(JObject.Parse(rawdata)["rates"]["CNY"].ToString()); ;


                                                double cnyusdrate =
                                                x = x * usdprice * eur2cny / eur2usd;
                                        ...
                                        {
                                            string url = @"https://www.okcoin.cn/api/v1/ticker.do?symbol=btc_cny";
                                            string rawdata = GetWebRequest(url);
                                            JObject ticker = JObject.Parse(JObject.Parse(rawdata)["ticker"].ToString());
                                            double pz = Convert.ToDouble(ticker["buy"].ToString());
                                            x = x * pz;
                                    ...
                                    {
                                        return (d / t).ToString("f2") + "T" + "(收益预测:ERR)";
                                    }
                                    return (d / t).ToString("f2") + "T" + "(收益预测:" + x.ToString("f2") + "CNY)";
                                }
                                else
